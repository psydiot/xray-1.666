...
IDirectDrawWrapper::IDirectDrawWrapper()
{
	...
	wsprintf((LPSTR)filename, TEXT("%s\\ddraw_settings.ini"), curPath);
	// Get windowed resolution
	GetPrivateProfileString(TEXT("video"), TEXT("windowedResolution"), TEXT("640x480"), (LPSTR)temp, 1024, (LPSTR)filename);
	...
	GetPrivateProfileString(TEXT("video"), TEXT("fullscreenResolution"), TEXT("640x480"), (LPSTR)temp, 1024, (LPSTR)filename);
	for(int i = 0; i < 1024 || temp[i] == TEXT('\0'); i++)
	{
		// Split on x or X
		if(temp[i] == TEXT('x') || temp[i] == TEXT('X'))
	...
	GetPrivateProfileString(TEXT("video"), TEXT("fullscreen"), TEXT("0"), (LPSTR)temp, 1024, (LPSTR)filename);
	if(temp[0] == TEXT('1'))
	{
		isWindowed = false;
	}
	...
	GetPrivateProfileString(TEXT("video"), TEXT("vsync"), TEXT("1"), (LPSTR)temp, 1024, (LPSTR)filename);
	if(temp[0] == TEXT('1'))
	{
		vSync = false;
	}
	...
	GetPrivateProfileString(TEXT("video"), TEXT("refresh"), TEXT("60"), (LPSTR)temp, 1024, (LPSTR)filename);
...
BOOL IDirectDrawWrapper::MenuKey(WPARAM vKey)
{
	...
	else if(vKey == VK_RETURN)
	{
		...
		wsprintf((LPSTR)filename, TEXT("%s\\ddraw_settings.ini"), curPath);
		temp[0] = '\0';
		wsprintf((LPSTR)temp, TEXT("%dx%d"), displayWidthWindowed, displayHeightWindowed);
		WritePrivateProfileString(TEXT("video"), TEXT("windowedResolution"), (LPSTR)temp, (LPSTR)filename);
		temp[0] = '\0';
		wsprintf((LPSTR)temp, TEXT("%dx%d"), displayWidthFullscreen, displayHeightFullscreen);
		WritePrivateProfileString(TEXT("video"), TEXT("fullscreenResolution"), (LPSTR)temp, (LPSTR)filename);
		temp[0] = '\0';
		wsprintf((LPSTR)temp, TEXT("%d"), refreshRate);
		WritePrivateProfileString(TEXT("video"), TEXT("refresh"), (LPSTR)temp, (LPSTR)filename);
		temp[0] = '\0';
		wsprintf((LPSTR)temp, TEXT("%d"), isWindowed ? 0 : 1);
		WritePrivateProfileString(TEXT("video"), TEXT("fullscreen"), (LPSTR)temp, (LPSTR)filename);
		temp[0] = '\0';
		wsprintf((LPSTR)temp, TEXT("%d"), vSync ? 1 : 0);
		WritePrivateProfileString(TEXT("video"), TEXT("vsync"), (LPSTR)temp, (LPSTR)filename);
	}
...
void IDirectDrawWrapper::DoSnapshot()
{
	...
	wchar_t title[1024];
	GetWindowText(hWnd, (LPSTR)title, 1024);
	wsprintf((LPSTR)filename, TEXT("%s\\%s_%.4d%.2d%.2d_%.2d%.2d%.2d.png"), curPath, title, sysTime.wYear, sysTime.wMonth, sysTime.wDay, sysTime.wHour, sysTime.wMinute, sysTime.wSecond);
	
	// Make sure it doesn't already exist
	WIN32_FIND_DATA findData;
	HANDLE findHandle = FindFirstFile((LPSTR)filename, &findData);
	int curFileNum = 1;
	while(findHandle != INVALID_HANDLE_VALUE)
	{
		...
		wsprintf((LPSTR)filename, TEXT("%s\\Diablo_%.4d%.2d%.2d_%.2d%.2d%.2d(%d).png"), curPath, sysTime.wYear, sysTime.wMonth, sysTime.wDay, sysTime.wHour, sysTime.wMinute, sysTime.wSecond, curFileNum);
		// Increment current file
		curFileNum++;
		// Check if this exists
		findHandle = FindFirstFile((LPSTR)filename, &findData);
	}

    // Save texture to file
	if(D3DXSaveTextureToFile((LPSTR)filename, D3DXIFF_PNG, surfaceTexture, NULL) != D3D_OK)
	{
...
bool IDirectDrawWrapper::CreateSurfaceTexture()
{
	...
	HRESULT hr = D3DXCreateTextureFromResource(d3d9Device, hModule, MAKEINTRESOURCE(""), &menuTexture);
	if(hr != D3D_OK)
	{
