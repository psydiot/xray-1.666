...
bool Script::bfLoadFileIntoNamespace(lua_State* tpLuaVM, LPCSTR caScriptName, LPCSTR caNamespaceName, bool bCall)
{
...
bool Script::bfGetNamespaceTable(lua_State* tpLuaVM, LPCSTR N)
{
    lua_pushstring(tpLuaVM, "_G");
    lua_gettable(tpLuaVM, LUA_GLOBALSINDEX);
    string256 S2;
...
lua_State* get_namespace_table(lua_State* tpLuaVM, LPCSTR N)
{
    if (!xr_strlen(N))
        return (tpLuaVM);
    lua_pushstring(tpLuaVM, "_G");
...
bool Script::bfIsObjectPresent(lua_State* tpLuaVM, LPCSTR identifier, int type)
{
    lua_pushnil(tpLuaVM);
    while (lua_next(tpLuaVM, -2))
    {
...
bool Script::bfIsObjectPresent(lua_State* tpLuaVM, LPCSTR namespace_name, LPCSTR identifier, int type)
{
    if (xr_strlen(namespace_name) && !bfGetNamespaceTable(tpLuaVM, namespace_name))
        return (false);
    return (Script::bfIsObjectPresent(tpLuaVM, identifier, type));
}
namespace luabind{
luabind::object lua_namespace_table(lua_State* tpLuaVM, LPCSTR namespace_name)
{
    string256 S1;
    xr_strcpy(S1, namespace_name);
    LPSTR S = S1;
    luabind::object lua_namespace = luabind::get_globals((lua_State*)tpLuaVM);
    for (;;)
    {
        if (!xr_strlen(S))
